 ...
 * POST /v1/x402/extract. Single URL or a batch (up to 10) for one payment.
 * The payer EOA signs a gasless EIP-3009 transferWithAuthorization (no tx
 * broadcast by the payer; the facilitator submits it and pays gas), and the
 * @x402/axios wrapper handles 402 -> sign PAYMENT-SIGNATURE -> retry.
 *
 * Usage:
 *   X402_CUSTOMER_PRIVATE_KEY=0x... npx tsx scripts/extract-pay.ts <url-or-json> [base-url]
 *
 * <url-or-json> is a single URL (https://...) or a JSON object, e.g.:
 *   https://example.com
 *   '{"urls":["https://example.com","https://example.org"],"schema":"title + key points"}'
 *
 * Env:
...
if (targetArg === undefined) {
  fail('usage: npx tsx scripts/extract-pay.ts <url-or-json> [base-url]');
}
if (rawKey === '') {
...
function payloadFrom(arg: string): unknown {
  const trimmed = arg.trim();
  if (trimmed.startsWith('{')) {
    try {
      return JSON.parse(trimmed) as unknown;
    } catch {
      fail('a JSON object argument must be valid JSON');
    }
  }
  return { url: trimmed };
}
const payload = payloadFrom(targetArg);
...
async function main(): Promise<void> {
  ...
  try {
    await axios.post(`${baseUrl}/v1/x402/extract`, payload);
  ...
  const res = await api.post('/v1/x402/extract', payload);
  if (res.status === 402) {
  ...
  const body = res.data as {
    results?: Array<{ url: string; status: string; error?: string; data?: { title?: string } }>;
    payment?: { payer?: string; priceUsdcUnits?: number };
  };
  console.log('served:     200 OK');
  for (const result of body.results ?? []) {
    const line = result.status === 'ok' ? 'ok    ' : 'error ';
    console.log(`  ${line} ${result.url}${result.error !== undefined ? ` — ${result.error}` : ''}`);
    if (result.status === 'ok') console.log(`        title=${result.data?.title ?? '(none)'}`);
  }
  ...
  console.log('PASS: x402 payment accepted, settled, structured extract served');
}
