...
| kkj x402 Trust Index | **INDEXED 2026-09-06 for the ngrok host** (auto-crawled `kkj-x402-trust-index/0.1`): capture id 46929 + extract id 46928, `observed_trust_score: 85.7, grade: A, verified_live: true`. Re-checked 2026-09-23: the 3 MB `trust-feed.json` contains **both ngrok URLs and nothing for `webcap.shoutsid.fyi`**, and there is no submission endpoint — their `/x402/changes` feed is sourced from the **CDP Bazaar** and polled hourly, so the new host can only appear after the Bazaar indexes it. The landing page no longer embeds the badges: they point at evidence keyed to a host we no longer advertise |
